...
class ConcurrencyManager:
    """
    Manages concurrent searches in files using ThreadPoolExecutor.
    """

    def __init__(self, keywords, file_paths, num_threads):
        if num_threads < 1:
    ...
    def run_searches(self):
        """
        Executes concurrent searches across provided file paths.
        """
    ...
    def get_results(self):
        """
        Returns the search results compiled from all threads.
        """
        return self.results


def main(keywords, file_paths, num_threads=4):
    try:
        start_time = time.time()
        manager = ConcurrencyManager(keywords, file_paths, num_threads)
...
if __name__ == "__main__":
    ...
    results = main(keywords, file_paths, num_threads)
    if results:
